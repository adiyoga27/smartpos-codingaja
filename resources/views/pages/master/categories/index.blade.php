...
@section('breadcrumb')
    <a href="{{ route('master.categories.index') }}" class="text-slate-400 hover:text-slate-600">Data Master</a>
    <span class="text-slate-400">/</span>
    <span class="text-slate-600">Kategori</span>
@endsection
@section('content')
<div class="flex items-center justify-between mb-4">
    <h4 class="font-bold mb-0">Kategori Produk</h4>
    <a href="{{ route('master.categories.create') }}" class="btn btn-primary btn-md"><i class="bi bi-plus-lg"></i> Tambah Kategori</a>
</div>
<div class="card">
    <div class="card-body">
        <div class="overflow-x-auto">
            <table class="table table-striped" id="categories-table" style="width:100%">
                <thead>
                    <tr><th>Kode</th><th>Nama</th><th>Deskripsi</th><th>Status</th><th>Produk</th><th>Aksi</th></tr>
                </thead>
            </table>
        </div>
    </div>
...
$(document).ready(function() {
    $('#categories-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route('master.categories.index') }}',
        pageLength: 25,
        dom: 'Bfrtip',
        buttons: [
