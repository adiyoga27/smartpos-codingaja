@extends('layouts.app')
@section('title', 'Buat Return Pembelian')
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('transaksi.purchase_returns.index') }}">Return Pembelian</a></li>
    <li class="breadcrumb-item active">Buat</li>
@endsection
@section('content')
<div class="card">
    <div class="card-header">Buat Return Pembelian</div>
    <div class="card-body">
        <form action="{{ route('transaksi.purchase_returns.store') }}" method="POST" id="returnForm">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-3">
                ...
                <div><label class="form-label">No. PO</label>
                    <select name="purchase_id" class="form-select" id="purchaseSelect" required>
                        <option value="">- Pilih PO -</option>
                        @foreach($purchases as $po)
                        <option value="{{ $po->id }}">{{ $po->document_number }} - {{ $po->supplier?->name ?? '-' }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div><label class="form-label">Refund</label>
                    <select name="refund_method" class="form-select" required><option value="credit">Kredit ke Hutang</option><option value="cash">Tunai</option></select>
                </div>
            </div>
            <div class="mb-3">
            ...
            <a href="{{ route('transaksi.purchase_returns.index') }}" class="btn btn-secondary btn-md">Batal</a>
        </form>
    </div>
</div>
<script>
    let purchases = @json($purchases->map(fn($p) => ['id'=>$p->id, 'items'=>$p->items->map(fn($i)=>['product_id'=>$i->product_id,'product_name'=>$i->product?->name ?? '-','unit_price'=>$i->unit_price])]));
    ...
    document.getElementById('purchaseSelect').addEventListener('change', function() {
        ...
        tbody.innerHTML = '';
        let po = purchases.find(p => p.id == this.value);
        if(!po) return;
