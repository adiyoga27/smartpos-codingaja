...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active text-slate-800 font-semibold" aria-current="page">Bayar</li>
@endsection

@section('content')
<div class="max-w-4xl mx-auto pb-10" x-data="payForm({{ $payable->remaining_amount }})">
    <!-- Header Section -->
    <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 flex items-center gap-2">
                <i class="bi bi-wallet2 text-primary-600"></i> Pembayaran Hutang
            </h2>
            <p class="text-sm text-slate-500 mt-1">Selesaikan pembayaran tagihan supplier sesuai nominal yang tersisa.</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <!-- Sisa Hutang Card -->
        <div class="bg-gradient-to-br from-red-500 to-rose-600 rounded-2xl p-5 border border-red-600 shadow-lg shadow-red-500/30 flex items-center gap-4 relative overflow-hidden text-white">
            ...
            <div class="z-10">
                <p class="text-[10px] uppercase font-bold text-red-100 tracking-wider mb-0.5">Sisa Hutang</p>
                <p class="text-lg font-black">{{ formatRupiah($payable->remaining_amount) }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden relative">
        ...
        <div class="p-5 sm:p-6 border-b border-slate-50">
            <h6 class="text-base font-bold text-slate-800 flex items-center gap-2 mb-1">
                <i class="bi bi-pencil-square text-primary-500"></i> Detail Pembayaran
            </h6>
        </div>
        
        <div class="p-5 sm:p-6">
            <form action="{{ route('keuangan.payables.pay.store', $payable) }}" method="POST" id="formBayar">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wider">Tanggal Bayar <span class="text-red-500">*</span></label>
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wider">Nominal Bayar <span class="text-red-500">*</span></label>
                        <div class="relative">
                            ...
                            <div class="absolute inset-y-0 right-0 pr-2 flex items-center">
                                <button type="button" @click="setLunas()" class="bg-primary-50 text-primary-600 hover:bg-primary-100 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors">Bayar Lunas</button>
                            </div>
                        </div>
                        <p class="text-xs text-slate-400 mt-2 flex items-center gap-1">
                            <i class="bi bi-info-circle"></i> Maksimal pembayaran: <span class="font-bold text-slate-600">{{ formatRupiah($payable->remaining_amount) }}</span>
                        </p>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wider">Keterangan / Catatan</label>
                        <textarea name="notes" class="w-full bg-white border border-slate-200 text-slate-700 text-sm rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all shadow-sm" rows="3" placeholder="Tambahkan catatan pembayaran jika perlu..."></textarea>
                    </div>
                </div>

                <div class="mt-8 pt-5 border-t border-slate-100 flex justify-end gap-3">
                    <a href="{{ route('keuangan.payables.index') }}" class="btn btn-light rounded-xl px-5 py-2.5 font-medium shadow-sm hover:bg-slate-100 transition-all">
                        Batal
                    </a>
                    <button type="submit" class="btn btn-primary bg-gradient-to-r from-primary-600 to-primary-500 rounded-xl px-6 py-2.5 shadow-lg shadow-primary-500/30 hover:-translate-y-0.5 transition-all border-0 flex items-center gap-2 font-bold" :disabled="amount <= 0 || amount > maxAmount">
                        <i class="bi bi-check2-circle text-lg"></i> Simpan Pembayaran
                    </button>
                </div>
            </form>
        </div>
    ...
    <div class="mt-6 bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden relative">
        ...
        <div class="p-5 sm:p-6 border-b border-slate-50">
            <h6 class="text-base font-bold text-slate-800 flex items-center gap-2 mb-1">
                <i class="bi bi-clock-history text-slate-500"></i> Riwayat Pembayaran
            </h6>
            <p class="text-xs text-slate-500">Histori cicilan atau pembayaran yang sudah dilakukan untuk hutang ini.</p>
        </div>
        <div class="p-0">
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-slate-600">
                    ...
                    <tfoot class="bg-slate-50 border-t border-slate-100">
                        <tr>
                            <td colspan="3" class="px-5 py-3 text-right font-bold text-slate-600">Total Telah Dibayar</td>
                            <td class="px-5 py-3 text-right font-black text-slate-800">{{ formatRupiah($payable->paid_amount) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('payForm', (maxRemaining) => ({
        ...
        formatInput(e) {
            ...
            // Prevent exceeding max amount visually (optional, but good UX)
            if(parsed > this.maxAmount) {
                if(typeof showToast === 'function') showToast('Maksimal pembayaran ' + this.formatNumber(this.maxAmount), 'warning');
